...
### Custom OS Images
With the release of version 1.2.0,BackToMac supports flashing custom OS images.Users can now create USB sticks with their own image files.(thanks to @Minh-Ton for the idea.)The flashing process is based on `dd`, so all formats that `dd` supports will work with BackToMac.
